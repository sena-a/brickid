...
async function productDetail(id) {
  ...
  const descriptionEl = frag
    .querySelector(".info .description")
    .querySelector("dd");
  const quantityEl = frag
    .querySelector(".info .quantity")
...
async function drawCart() {
  ...
  for (const cartItem of cartItemList) {
    ...
    // 3. 필요한 데이터 불러오기
    const itemProduct = product.find(
      item => item.id === cartItem.option.productId
    );
...
async function drawOrder() {
  ...
  // 4. 내용 채우기
  const reverseList = orderList.reverse();
  for (const eachOrdered of reverseList) {
    // 1. 템플릿 복사
    ...
    // 4. 내용 채우기
    orderedIdEl.textContent = orderList.length - orderList.indexOf(eachOrdered);
